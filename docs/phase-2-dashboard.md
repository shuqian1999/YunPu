...
1. **仪表盘页面** (`Dashboard.vue`)
   - 统计卡片背景色适配（人物总数、事件总数、提醒总数）
   - 事件时间轴卡片深色模式适配
   - 最近30天提醒列表深色模式适配
   - 使用 `:deep()` 选择器覆盖 Element Plus 组件样式

2. **人物管理页面** (`Persons.vue`)
   - 搜索栏输入框深色模式配色修复
   - 人物卡片背景色和文字颜色适配
   - 卡片边框和阴影主题适配

3. **人物详情页面** (`PersonDetail.vue`)
   - 信息卡片深色模式适配
   - 关系列表深色模式适配
   - 编辑状态下输入框、日期选择器、选择框样式修复
   - 按钮和标签组件主题适配
...
- 使用 CSS 变量统一管理主题色（`variables.scss`）
- 添加 Element Plus 组件深色模式变量覆盖
- 使用 `:deep()` 选择器解决样式作用域问题
- 使用 `!important` 强制覆盖组件默认样式
